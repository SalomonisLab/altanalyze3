...
def _load_isoform_filtered_matrix(kind):
    ...
    `kind` is 'cpm' or 'ratio'. The isoform analyses are H5AD-ONLY: read the compact h5ad
...
def _load_gene_cpm_matrix():
    """Load the study-combined GENE CPM pseudobulk (genes x cluster-sample) -- EXACTLY mirroring the
    isoform path. The gene combine wrote 'gene_combined_pseudo_cluster_cpm.h5ad' using the same CPM
    normalization the isoform combine uses (counts / column_total * 1e6 = CP1M), and ALL genes are
    retained (the FULL, not -filtered, h5ad). The caller then applies the identical downstream
    log2(matrix + 1), so gene fold-changes are on the same scale as isoform fold-changes.

    (Previously this applied cellHarmony log2-CP10k, an EXTRA per-column 1e4 normalization that
    compressed the range ~100x vs the isoform log2(CPM+1); switched to plain CPM to match isoform.)"""
    h5ad_path = 'gene_combined_pseudo_cluster_cpm.h5ad'
    if not os.path.exists(h5ad_path):
        raise FileNotFoundError(
            f"gene CPM pseudobulk h5ad not found: {h5ad_path}. Run the gene combine "
            f"(isoform_automate.combine_genes, compute_cpm) before gene differentials.")
    import anndata as ad
    a = ad.read_h5ad(h5ad_path)
    X = a.X.toarray() if hasattr(a.X, 'toarray') else np.asarray(a.X)
    # obs = cluster-sample (rows), var = genes (cols) -> genes x cluster-sample = X.T
    return pd.DataFrame(X.T, index=list(a.var_names), columns=list(a.obs_names))
...
def _resolve_uid(raw_uid, uid_to_group, data_type):
    ...
    if raw_uid.endswith(suffix):
        ...
        if trimmed in uid_to_group:
            return trimmed
    for fallback in ("-junction", "-isoform", "-ratio", "-gene"):
...
def compare_one_cluster_to_many(condition,cluster_order,groups_file,psi_matrix,junction_coords_file,gene_symbol_file,dataType='junction',method='mwu'):
    ...
    if dataType == 'gene':
        stats_folder = 'diff-cluster-gene/'
    ...
    # GENE differentials are stats-only (no isoform/protein structure to annotate).
    if dataType == 'gene':
        print('Gene differentials complete (stats only, no annotation).')
        return
...
def compare_two_groups_per_cluster(condition1,condition2,cluster_order,groups_file,psi_matrix,junction_coords_file,gene_symbol_file,dataType='junction',method='mwu'):
    ...
    if dataType == 'gene':
        stats_folder = 'diff-covariate-gene/'
    _reset_stats_folder(stats_folder)  # archive stale per-cluster stats so annotate sees only this run
    ...
    # GENE differentials are stats-only (no annotation).
    if dataType == 'gene':
        print('Gene differentials complete (stats only, no annotation).')
        return
    print ('Annotating the test results by gene, isoform and protein...')
...
def compute_differentials(sample_dict,conditions,cluster_order,gene_symbol_file,analyses=['junction','isoform','isoform-ratio'],method='mwu'):
    ...
    for pair in conditions:
        ...
        if 'isoform' in analyses:
            ...
            # Source: the filtered CPM pseudobulk h5ad (falls back to legacy -filtered.txt).
            isoform_matrix = _load_isoform_filtered_matrix('cpm')
        ...
        if 'gene' in analyses:
            # Gene-level differential expression -- EXACTLY mirroring the isoform path: CPM pseudobulk
            # (all genes retained) -> log2(CPM + 1). Same metaDataAnalysis stats engine; stats only, no
            # annotation. Outputs: diff-cluster-gene/ and diff-covariate-gene/.
            gene_matrix = _load_gene_cpm_matrix()
            log2_gene_matrix = np.log2(gene_matrix + 1)
            uid_to_group = isoa.get_sample_to_group(sample_dict, 'gene')
            groups_file = pd.DataFrame({
                'grp': [uid_to_group.get(_resolve_uid(col.split('.', 1)[1], uid_to_group, 'gene'), 'unknown') for col in gene_matrix.columns],
                'uid': gene_matrix.columns
            }).set_index('uid')
            if condition1 not in analyzed_intial_conditions:
                compare_one_cluster_to_many(condition1,cluster_order,groups_file,log2_gene_matrix,None,gene_symbol_file,dataType='gene',method=method)
            compare_two_groups_per_cluster(condition1,condition2,cluster_order,groups_file,log2_gene_matrix,None,gene_symbol_file,dataType='gene',method=method)
