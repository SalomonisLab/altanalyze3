...
def exportRatios(ob,out):
    pseudo_pdf, cpm, isoform_to_gene_ratio = iso.pseudo_cluster_counts(ob,cell_threshold=5,count_threshold=0,compute_cpm=True)
    ...
    for _, row in metadata.iterrows():
        ...
        if 'bam' in row:
            ...
            if extract_from_bams:
                ...
                del stats, n, o
                _trim_memory()
                # Gene-level aggregate of the molecule h5ad -> '<library>-gene.h5ad' beside it, so the
                # later gene differentials (sclr-diff --analyses gene) find it with no extra step.
                # Idempotent (skips if it already exists) and best-effort (never fails extraction).
                _ensure_gene_h5ad(matrix_path)
            else:
...
def export_pseudo_counts(metadata_file,barcode_cluster_dirs,dataType='junction',compute_cpm=False,only_import_existing=False,fused_filter=None):
    ...
    # junction path (the 121GB bottleneck), OFF for isoform (which filters counts/cpm/ratio with
    # independent masks -- left on the legacy two-step path for now). The CPM/ratio filtered files for
    ...
    pseudo_cpm = dataType+'_combined_pseudo_cluster_cpm.txt'
    ...
    iso.concatenate_h5ad_and_compute_pseudobulks_optimized(
        sample_files, collection_name=dataType, compute_cpm=compute_cpm,
    ...
    # Organize and filter.
    #   * ISOFORM: the combine wrote counts/cpm/ratio as h5ad (full + filtered) directly from the
...
def combine_genes(metadata_file, barcode_cluster_dirs):
    """P-combine for GENE: concatenate the PER-SAMPLE gene pseudobulks (already built in parallel by
    export_sample_gene / sclr-gene-aggregate) into the study-combined gene pseudobulk h5ad. NO
    per-sample work here, and NO '_gene_clustered/' copies -- the per-sample '<library>-gene.txt'
    (+_cpm) live beside each '<library>-gene.h5ad' (the proper local dir, with the isoform/junction
    outputs); this step only sums across samples.

    Exactly mirrors combine_isoforms (only_import_existing=True). compute_cpm=True writes the CPM
    pseudobulk (counts/row_total * 1e6) so the gene differentials mirror the isoform method
    (log2(CPM + 1)). Writes gene_combined_pseudo_cluster_counts.h5ad AND
    gene_combined_pseudo_cluster_cpm.h5ad (the FULL CPM matrix -- ALL genes retained)."""
    export_pseudo_counts(metadata_file, barcode_cluster_dirs, 'gene', compute_cpm=True,
                         only_import_existing=True)


def _find_sample_gene_h5ad(directory, library):
    """Locate the ORIGINAL per-sample '<library>-gene.h5ad' (the cluster-less aggregate).

    NEVER return a copy under '_gene_clustered/' -- that is THIS module's own cluster-annotated
    output (it already carries obs['cluster']). The recursive glob below would otherwise match it on
    a re-run, and re-joining 'cluster' raises 'columns overlap but no suffix specified'."""
    if not directory:
        return None
    ...
    if not os.path.isdir(directory):
        return []
    hits = [h for h in glob.glob(os.path.join(directory, "**", f"{library}-gene.h5ad"), recursive=True)
            if "_gene_clustered" not in os.path.normpath(h).split(os.sep)]
    return hits[0] if hits else None


def _gene_h5ad_path_for(matrix_path):
    """The gene-level aggregate path for a molecule h5ad: '<prefix>-gene.h5ad' beside it
    (the same name aggregate_to_gene_level writes and _find_sample_gene_h5ad looks for)."""
    matrix_path = str(matrix_path)
    return (matrix_path[:-5] if matrix_path.endswith('.h5ad') else matrix_path) + '-gene.h5ad'


def _ensure_gene_h5ad(matrix_path):
    """Aggregate a molecule h5ad to gene level -> '<prefix>-gene.h5ad' beside it, so the gene
    differentials (sclr-diff --analyses gene) find it without a separate aggregation step.

    Idempotent: returns immediately if the gene h5ad already exists (so re-running BAM extraction,
    or a run where it was produced elsewhere, does no redundant work -- the "check cellHarmony and
    skip if present" behavior). Best-effort: any failure is logged, NOT raised, so it can never break
    ...
    try:
        ...
        if not os.path.exists(str(matrix_path)):
            print(f"[gene-agg] molecule h5ad missing, cannot aggregate: {matrix_path}", flush=True)
            return None
        from ..aggregate.gene_aggregate import aggregate_to_gene_level
        return aggregate_to_gene_level(str(matrix_path), output_path=out)
    except Exception as exc:  # never fail extraction on the optional gene aggregate
        print(f"[gene-agg] WARNING: gene aggregation failed for {matrix_path}: {exc}", flush=True)
        return None


def export_sample_gene(sample_entry_list, barcode_sample_dict, uid=None):
    """PER-SAMPLE (parallel fan-out): build ONE uid's gene-level pseudobulk beside its gene h5ad.

    For each library: ensure '<library>-gene.h5ad' exists (aggregate from the molecule h5ad if not),
    join the barcode->cluster labels onto its obs IN MEMORY (gene h5ads carry no obs['cluster']),
    honoring the per-sample reverse-complement flag, then write the per-sample pseudobulk
    '<library>-gene.txt' (+ '<library>-gene_cpm.txt') beside the gene h5ad -- the SAME local directory
    as the isoform/junction outputs. NO cluster-annotated gene h5ad copy is written anywhere (no
    '_gene_clustered/'); the annotation lives only in memory for the duration of the pseudobulk.

    The cross-sample combine (combine_genes, only_import_existing=True) then just concatenates these
    per-sample '.txt' files -- no per-sample work. This is the gene analogue of export_sample_isoform,
    so it fans out as one bsub per uid exactly like sclr-isoquant."""
    for s in sample_entry_list:
        lib = s.get('library') or s.get('gff_name') or uid
        mol = str(s.get('matrix') or '')
        gene_h5ad = _ensure_gene_h5ad(mol)  # idempotent; aggregates if missing
        if not gene_h5ad or not os.path.exists(gene_h5ad):
            gene_h5ad = _find_sample_gene_h5ad(os.getcwd(), lib) or _find_sample_gene_h5ad(
                os.path.dirname(s.get('gff', '')), lib)
        if not gene_h5ad:
            raise FileNotFoundError(f"gene h5ad not found/aggregatable for library '{lib}'")
        adata = ad.read_h5ad(gene_h5ad)
        if 'cluster' in adata.obs.columns:        # idempotent: drop any stale cluster before the join
        ...
        bc.name = 'cluster'
        adata = iso.calculate_barcode_match_percentage(
            adata, bc, context=f"gene h5ad '{lib}' (reverse={bool(s.get('reverse'))})")
        # Write the per-sample pseudobulk '<prefix>-gene.txt' (+_cpm) beside the gene h5ad.
        prefix = gene_h5ad[:-5] if gene_h5ad.endswith('.h5ad') else gene_h5ad
        iso.pseudo_cluster_counts_optimized(prefix, adata, cell_threshold=0, count_threshold=0,
                                            compute_cpm=True, cpm_threshold=1, status=False)
        print(f"[gene] {lib}: {adata.shape[0]} cells x {adata.shape[1]} genes "
              f"-> {os.path.basename(prefix)}.txt", flush=True)
        del adata
        _trim_memory()
...
def _pseudobulk_one_h5ad(h5ad_path, dataType, compute_cpm):
    """Compute the per-sample pseudobulk <prefix>.txt (+_cpm/_ratio for isoform) for ONE sample h5ad,
...
def export_sample_junctions(sample_entry_list, ensembl_exon_dir, barcode_sample_dict, uid=None):
    ...
    if num_samples > 1:
        ...
        _trim_memory()
        _pseudobulk_one_h5ad(uid_h5ad, 'junction', compute_cpm=False)
    else:
        del adata_list
        ...
        _pseudobulk_one_h5ad(per, 'junction', compute_cpm=False)
...
def export_sample_isoform(sample_entry_list, gff_output_dir, barcode_sample_dict, uid=None,
    ...
    if len(sample_entry_list) > 1:
        ...
        del adatas, combined
        _trim_memory()
        _pseudobulk_one_h5ad(uid_h5ad, 'isoform', compute_cpm=compute_cpm)
    else:
        _pseudobulk_one_h5ad(out_h5ads[0], 'isoform', compute_cpm=compute_cpm)
...
def combine_processed_samples(metadata_file, barcode_cluster_dirs, ensembl_exon_dir, gencode_gff, genome_fasta,
    ...
    export_pseudo_counts(metadata_file,barcode_cluster_dirs,'isoform',compute_cpm=True)
