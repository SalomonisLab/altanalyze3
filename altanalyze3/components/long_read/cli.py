...
def run_sclr_isoquant(args):
    ...
    for uid, libs in sample_dict.items():
        isoa.export_sample_isoform(libs, gff_output_dir, barcode_sample_dict, uid=uid,
                                   collapse_method=collapse_method, compute_cpm=True)
...
def run_sclr_gene_aggregate(args):
    """Per sample (parallel, the gene analogue of sclr-isoquant): build ONE uid's gene-level
    pseudobulk. For each library it ensures '<library>-gene.h5ad' exists (aggregating from the
    molecule h5ad if needed), joins the barcode->cluster labels IN MEMORY (from --cell_annot, same
    source the isoform/junction diffs use), and writes the per-sample pseudobulk
    '<library>-gene.txt' (+_cpm) beside the gene h5ad -- the same local dir as the isoform/junction
    outputs. NO '_gene_clustered/' copies. With --sample, processes one uid (fan out one bsub per uid);
    without, loops all uids. Idempotent. The combine (sclr-diff --analyses gene) then just concatenates
    ...
    for uid, libs in sample_dict.items():
        isoa.export_sample_gene(libs, barcode_sample_dict, uid=uid)
    logging.info("sclr-gene-aggregate: per-sample gene pseudobulk complete for %s.",
                 args.sample if getattr(args, "sample", None) else "all samples")
...
def run_sclr_diff(args):
    ...
    if not analyses:
        raise ValueError("--analyses must include at least one of: junction, isoform, isoform-ratio, gene.")
    wants_iso_abundance = 'isoform' in analyses          # reads the combined CPM matrix
    wants_iso_ratio = 'isoform-ratio' in analyses         # reads the combined RATIO matrix
    wants_gene = 'gene' in analyses
    # Only junction + isoform(+ratio) differentials are annotated (and need the P3 protein summary).
    # Gene-only diffs are stats-only and require NO P3 product.
    wants_annotated = any(a in ('junction', 'isoform', 'isoform-ratio') for a in analyses)
    ...
    # Combine the per-sample isoform pseudobulks into the study matrices ONLY when an isoform analysis
    # is requested AND the matrix it reads is MISSING. combine_isoforms writes ALL THREE combined h5ads
    # (counts/cpm/ratio) in one pass, so if the needed one already exists (e.g. the isoform diff already
    # ran and we are now adding ONLY the ratio diff), do NOT re-run it -- that would needlessly rewrite
    # the large isoform_combined_pseudo_cluster_*.h5ad files and rebuild the viewer index. Each
    # differential below reads its matrix off disk (CPM for 'isoform', RATIO for 'isoform-ratio').
    iso_cpm_h5ad = 'isoform_combined_pseudo_cluster_cpm-filtered.h5ad'
    iso_ratio_h5ad = 'isoform_combined_pseudo_cluster_ratio-filtered.h5ad'
    need_combine = (
        (wants_iso_abundance and not os.path.exists(iso_cpm_h5ad)) or
        (wants_iso_ratio and not os.path.exists(iso_ratio_h5ad))
    )
    if need_combine:
        isoa.combine_isoforms(str(args.metadata), barcode_cluster_dirs)
    elif wants_iso_abundance or wants_iso_ratio:
        logging.info("sclr-diff: combined isoform matrices already present -- skipping combine_isoforms.")
    # Gene-level differentials: build the study-combined gene pseudobulk h5ad (all genes retained) from
    # the per-sample '<library>-gene.h5ad' files. Stats-only, no annotation.
    if wants_gene:
        isoa.combine_genes(str(args.metadata), barcode_cluster_dirs)
    ...
    # ratio-only, gene-only, and junction-only diffs. Best-effort + idempotent.
