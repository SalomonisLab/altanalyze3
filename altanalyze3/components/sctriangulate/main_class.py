...
import gseapy as gp
from ._compat import scrublet as scr   # deferred: only used by doublet_predict()
...
def _require_enrichment(marker_genes_df,column,who):
    '''
    Fail with a readable message when the artifact enrichment columns are absent.

    ``marker_gene`` skips the enrichr and GSEA columns by default, because they
    feed no metric and no label and the GSEA permutations dominate the runtime.
    The handful of plotting and penalising entry points that DO read them land
    here instead of on a bare KeyError.
    '''
    if column in marker_genes_df.columns:
        return
    raise KeyError(
        "{} needs the '{}' column of uns['marker_genes'], which this run did not compute. "
        "Re-run with --enrichment on the command line, or set "
        "altanalyze3.components.sctriangulate.metrics.RUN_ENRICHMENT = True before "
        "compute_metrics.".format(who,column))


SPARSE_COMPUTE = True
'''
Keep adata.X sparse through the metric steps.

Upstream densified X because scanpy and sklearn take different code paths for
sparse input. That is no longer true here: the group statistics come from the
patched `_basic_stats`, which uses one formula for both, tf-idf counts nonzeros
from the sparse structure, and SCCAF hands the sparse matrix straight to
liblinear. reassign_score is the one step that needs dense, and only for the few
thousand marker genes it selects.

Verified on the demo: sparse and dense give identical rank_genes_groups names and
scores, and identical marker lists for all 61 clusters. At 984,119 by 36,249 the
dense matrix is 142.7 GB; the CSR at 5% nonzero is 14.3 GB.

Set False, or pass --dense-compute, to restore the densified path.
'''


def _cleanup_enrichr_tmp(directory):
    '''
    No-op stand-in for upstream's ``subprocess.run(['rm','-r', <dir>])``.

    Upstream shelled out to ``rm -r`` to delete the ``scTriangulate_local_mode_enrichr``
    scratch directory that ``gseapy.enrichr(outdir=...)`` produced. Two reasons
    this is now a no-op:

      * ``metrics.run_enrichr`` computes the hypergeometric test in-process and
        writes nothing, so the directory is never created. The ``rm -r`` only
        ever printed "No such file or directory" to stderr.
      * Recursive deletion driven by a formatted path is not something this
        package should do on a user's disk.

    If a stale directory from an older run exists, its path is reported so the
    user can remove it themselves.
    '''
    stale = os.path.join(directory,'scTriangulate_local_mode_enrichr')
    if os.path.isdir(stale):
        logger_sctriangulate.info(
            'leftover scratch directory from a previous upstream run, not deleting: {}'.format(stale))


def _n_workers(requested,upper):
    '''Worker count clamped to available cores; never below 1.'''
    if requested is not None:
        return max(1,int(requested))
    return max(1,min(int(upper),mp.cpu_count()))


def _get_mp_context():
    '''
    Multiprocessing context that behaves on macOS.

    Python 3.8+ defaults macOS to the 'spawn' start method. Under spawn every
    worker re-imports scanpy/anndata/matplotlib (seconds each) and every
    argument is pickled, so upstream's ``pool.apply_async(each_key_run, (self, ...))``
    shipped a full copy of the AnnData to each worker. 'fork' shares the parent's
    pages copy-on-write and starts instantly.

    Fork is only safe before threads exist, and Accelerate/OpenMP thread pools
    in this process would make a forked child deadlock. The caller therefore
    pins BLAS to one thread inside the workers (see ``_single_threaded_blas``).
    Set SCTRI_MP_START=spawn to override.
    '''
    want = os.environ.get('SCTRI_MP_START')
    if want:
...
class _single_threaded_blas:
    '''
    Context manager pinning BLAS/OpenMP to one thread.

    Needed when q annotations run in q processes: without it each worker's
    Accelerate/OpenMP pool also claims every core, and the resulting
    oversubscription is slower than running sequentially.
    '''
...
class ScTriangulate(object):
    ...
    Four attributes and two methods exist here that upstream does not have. All
    four attributes default to the behaviour that reproduces upstream's numbers,
    so an untouched object runs the upstream workflow.

    :ivar sparse_compute: boolean, default from the module constant
        ``SPARSE_COMPUTE`` (True). Keep X sparse through every metric. Upstream
        densified: 142.7 GB for a 984,119 by 36,249 matrix. Measured identical
        labels (ARI 1.0000) with 2.5x less memory, so it is the default. Set it
        to False to take the dense path.
    :ivar run_enrichment: boolean, default False. Add the enrichr and GSEA
        artifact annotation of the marker genes. It feeds no stability metric
        and no Shapley decision, and costs about 3.7 s of a 21 s demo run. The
        viewer's per-cluster page needs it.
    :ivar sccaf_mode: ``'optimized'`` (default) or ``'legacy'``. See
        ``metrics.SCCAF_score``. Both give the same accuracies; only the matrix
        representation differs.
    :ivar size_dict: cluster sizes per annotation, measured at ``__init__``.
        It is the denominator of ``win_fraction`` and ``proportion_to_self``.
        ``downsample_cells`` recomputes it; anything else that drops cells must
        do the same or every cluster looks like it lost cells it never had.

    Two optional reductions, both OFF, both to be called BEFORE
    ``compute_metrics`` or ``lazy_run``, and both changing what every metric
    sees. Call ``downsample_cells`` first when using both, so the marker ranking
    reads the cells that survive:

    * ``downsample_cells(max_per_cluster)`` caps cells per cluster using one
      whitelist shared across annotations. See ``prefilter.downsample_indices``.
    * ``prefilter_marker_genes(n_per_cluster)`` keeps the union of the top
      MarkerFinder genes per cluster. See ``prefilter.select_marker_genes``.
      Circular by construction and measured to move the labels; read the table
      in README.md before turning it on.

    Both record what they did in ``self.uns['downsample']`` and
    ``self.uns['prefilter']``, which the CLI copies into ``run_parameters.json``.

    Example with both reductions::

        sctri = ScTriangulate(dir='./output',adata=adata,query=['ann1','ann2'])
        sctri.downsample_cells(max_per_cluster=1000)      # cells first
        sctri.prefilter_marker_genes(n_per_cluster=200)   # then genes
        sctri.lazy_run()
    ...
    def __init__(self,dir,adata,query,species='human',criterion=2,verbose=1,reference=None,add_metrics={'tfidf5':tf_idf5_for_cluster},
        ...
        # None means "read metrics.RUN_ENRICHMENT at call time". Set it on the
        # instance to make the choice travel with the object into worker processes.
        self.run_enrichment = None
        # None means "read metrics.SCCAF_MODE at call time"; set it on the
        # instance so the choice travels into worker processes
        self.sccaf_mode = None
    ...
    def downsample_cells(self,max_per_cluster=1000,seed=0):
        '''
        Keep at most ``max_per_cluster`` cells per cluster, sharing one whitelist
        across every annotation.

        The first annotation draws its full quota. Every later annotation counts
        the cells the whitelist already holds for each of its own clusters and
        draws only the shortfall, so annotations that agree do not each pay for
        the same cells. See prefilter.downsample_indices.

        Run this BEFORE compute_metrics. Cell counts are the denominator of
        every stability metric, so this is not the same experiment as a full run
        and it is off by default.

        :return: the info dict, which the CLI copies into run_parameters.json.
        '''
        from .prefilter import downsample_indices
        keys = list(self.query)
        if self.reference not in keys:
            keys.append(self.reference)
        rows,info = downsample_indices(self.adata.obs,keys,max_per_cluster,seed=seed)
        if len(rows) == self.adata.shape[0]:
            logger_sctriangulate.info('downsample: every cluster is at or below the cap, nothing removed')
            self.uns['downsample'] = info
            return info
        self.adata = self.adata[rows,:].copy()
        # size_dict was measured on the full object at __init__ and is the
        # denominator of win_fraction and proportion_to_self; a stale one would
        # make every cluster look like it lost cells it never had
        self.size_dict, _ = get_size(self.adata.obs,self.query)
        self.uns['downsample'] = info
        return info

    def prefilter_marker_genes(self,n_per_cluster=200,max_cells_per_cluster=1000,seed=0):
        '''
        Subset the object to the union of the top MarkerFinder genes per cluster.

        Run this BEFORE compute_metrics. It changes the input to every metric,
        so it is not on by default; see prefilter.py and
        benchmarks/prefilter_validate.py.

        :return: the info dict, which the CLI copies into run_parameters.json.
        '''
        from .prefilter import select_marker_genes
        genes,info = select_marker_genes(self.adata,self.query,
                                         n_per_cluster=n_per_cluster,
                                         max_cells_per_cluster=max_cells_per_cluster,
                                         seed=seed)
        if not genes:
            raise RuntimeError('prefilter selected no genes; check the annotations')
        keep = self.adata.var_names.isin(set(genes))
        self.adata = self.adata[:,keep].copy()
        self.uns['prefilter'] = info
        return info

    def _install_sparse_sidecar(self):
        '''
        Park the sparse X in a layer before anything densifies it.

        SCCAF wants a sparse matrix; every other metric wants the dense one that
        upstream produced. Converting back and forth costs a full pass over the
        matrix each time. Keeping the CSR in a layer costs its own size, 27 MB
        on the demo against 354 MB dense, and AnnData keeps its rows aligned
        when the object is sliced into a view.
        '''
        from .metrics import SPARSE_SIDECAR_LAYER
        try:
            if issparse(self.adata.X):
                self.adata.layers[SPARSE_SIDECAR_LAYER] = self.adata.X
            elif SPARSE_SIDECAR_LAYER not in self.adata.layers:
                self.adata.layers[SPARSE_SIDECAR_LAYER] = csr_matrix(self.adata.X)
        except Exception as e:
            # never fail the run over an optimisation; SCCAF converts on demand
            logger_sctriangulate.info(
                'could not park a sparse copy of X ({}: {}); SCCAF will convert on demand'.format(
                    type(e).__name__, e))

    def _remove_sparse_sidecar(self):
        from .metrics import SPARSE_SIDECAR_LAYER
        if SPARSE_SIDECAR_LAYER in self.adata.layers:
            del self.adata.layers[SPARSE_SIDECAR_LAYER]

    def _to_dense(self):
        # upstream called .toarray() unconditionally, which raises AttributeError
        # if X is already dense
        if issparse(self.adata.X):
    ...
    def salvage_run(step_to_start,last_step_file,outdir=None,scale_sccaf=True,layer=None,added_metrics_kwargs=[{'species':'human','criterion':2,'layer':None}],compute_shapley_parallel=True,
        ...
        if step_to_start == 'assess_pruned':
            ...
            if assess_pruned:
                ...
                _cleanup_enrichr_tmp(sctri.dir)
    ...
    def compute_metrics(self,parallel=True,scale_sccaf=False,layer=None,added_metrics_kwargs=[{'species': 'human', 'criterion': 2, 'layer': None}],cores=None):
        ...
        '''
        self._install_sparse_sidecar()
        if parallel and len(self.query) > 1:
            cores = _n_workers(cores,len(self.query))
            ctx = _get_mp_context()
            start_method = ctx.get_start_method()
            logger_sctriangulate.info(
                'Spawn to {} processes using the {} start method'.format(cores,start_method))
            sparse_compute = getattr(self,'sparse_compute',SPARSE_COMPUTE)
            if sparse_compute:
                # nothing downstream needs a dense X, and fork shares the CSR
                # copy-on-write just as well as it shared the dense one
                self._to_sparse()
            elif start_method == 'fork':
                # Densify ONCE in the parent. each_key_run needs a dense X, and
                # fork shares the parent's pages copy-on-write. Nothing in the
                # metric functions writes to X -- sklearn's scale() and fancy
                # indexing both return new arrays -- so the one buffer stays
                # shared and the workers cost nothing for it. Leaving X sparse
                # here made every worker call .toarray() and allocate its own
                # copy: 640 MB each on an 8,000 by 20,000 matrix, so 1.9 GB of
                # duplicate pages across three workers.
                self._to_dense()
            else:
                # spawn pickles the object, so ship the small representation
                self._to_sparse()
            with _single_threaded_blas():
                if start_method == 'fork':
                    # publish the object as a module global BEFORE forking, so
                    # children inherit it copy-on-write. Pool.apply_async pickles
                    # its arguments whatever the start method, so upstream's
                    # `args=(self, ...)` shipped a full AnnData per worker.
                    global _FORK_SHARED_SCTRI
                    _FORK_SHARED_SCTRI = self
                    try:
                        pool = ctx.Pool(processes=cores)
                        try:
                            raw_results = [pool.apply_async(_each_key_run_forked,args=(key,scale_sccaf,layer,added_metrics_kwargs)) for key in self.query]
                            pool.close()
                            pool.join()
                            raw_results = [r.get() for r in raw_results]
                        finally:
                            pool.terminate()
                    finally:
                        _FORK_SHARED_SCTRI = None
                else:
                    pool = ctx.Pool(processes=cores)
                    try:
            ...
            _cleanup_enrichr_tmp(self.dir)
            self._to_sparse()
            self._remove_sparse_sidecar()

        else:
            logger_sctriangulate.info('choosing to compute metrics sequentially')
            if getattr(self,'sparse_compute',SPARSE_COMPUTE):
                self._to_sparse()
    ...
    def penalize_artifact(self,mode,stamps=None,parallel=True):
        ...
        if mode == 'void':
            ...
            for key,clusters in self.cluster.items():
                _require_enrichment(marker_genes[key],'gsea',"penalize_artifact(mode='cellcycle')")
    ...
    def compute_shapley(self,parallel=True,mode='shapley_all_or_none',bonus=0.01,cores=None):
        ...
        '''
        # -----------------------------------------------------------------
        # Batched Shapley. The `parallel` and `cores` arguments are accepted for
        # API compatibility but are no longer used: upstream split the cells
        # across processes because each cell cost O(n * 2**(n-1)) coalitions,
        # and on macOS (spawn start method) every worker re-pickled the whole
        # AnnData. The closed-form engine in shapley.py evaluates the same sum
        # per distinct score matrix, which is fast enough single-process and
        # avoids the pickling entirely.
        # -----------------------------------------------------------------
        if parallel is False:
            logger_sctriangulate.info('compute_shapley: running the batched engine (parallel flag ignored)')
        ...
        '''
        data = np.empty([len(self.query), obs.shape[0], len(score_colname)])
        for i, key in enumerate(self.query):
            practical_colname = [name + '@' + key for name in score_colname]
            data[i, :, :] = obs[practical_colname].values

        final, intermediate = run_shapley(obs, self.query, self.reference,
                                          self.size_dict, data, mode, bonus)
        ...
        self.adata.obs = run_assign(self.adata.obs)

        # prefixing
        self._prefixing(col='raw')

    def _prefixing(self,col):
        # vectorised string concat; upstream looped cell by cell
        col1 = self.adata.obs[col].astype('str').to_numpy(dtype=object)
        col2 = self.adata.obs[self.reference].astype('str').to_numpy(dtype=object)
        self.adata.obs['prefixed'] = self.reference + '@' + col2 + '|' + col1
    ...
    def plot_cluster_feature(self,key,cluster,feature,enrichment_type='enrichr',save=True,format='pdf'):
     ...
     '''
        if feature == 'enrichment':
            _require_enrichment(self.uns['marker_genes'][key],enrichment_type,
                                "plot_cluster_feature(feature='enrichment')")
            fig,ax = plt.subplots()
...
def penalize_artifact_void(obs,query,stamps,metrics):
    '''
    Zero every metric column belonging to a stamped cluster.

    Upstream rebuilt the whole ``annotation@cluster`` label matrix, repeated it
    across metrics and re-wrote the metric block once PER STAMP, so the cost was
    O(n_stamps * n_cells * n_query). A cell's label can match at most one stamp,
    so testing membership in the stamp set once gives the same result in a
    single pass.
    '''
    if not stamps:
        return obs
    metric_cols = [item2+'@'+item1 for item1 in query for item2 in metrics]
    stamp_set = set(stamps)
    # labels[:, i] is 'query[i]@<that cell's cluster>' ; each label column is
    # repeated len(metrics) times to line up with metric_cols
    labels = np.column_stack([
        np.char.add(key + '@', obs[key].astype('str').to_numpy(dtype=str))
        for key in query])
    hit = np.isin(labels,list(stamp_set))
    hit_repeat = np.repeat(hit,len(metrics),axis=1)
    values = obs.loc[:,metric_cols].to_numpy(dtype=np.float64,copy=True)
    values[hit_repeat] = 0
    obs.loc[:,metric_cols] = values
    return obs



# Set by compute_metrics just before forking, read by the workers through
# copy-on-write. Never pickled, and cleared as soon as the pool is done.
_FORK_SHARED_SCTRI = None


def _each_key_run_forked(key,scale_sccaf,layer,added_metrics_kwargs=None):
    '''Worker entry point for the fork backend: reads the object from the inherited global.'''
    if _FORK_SHARED_SCTRI is None:
        raise RuntimeError('_FORK_SHARED_SCTRI was not published before forking')
    return each_key_run(_FORK_SHARED_SCTRI,key,scale_sccaf,layer,added_metrics_kwargs)
...
def each_key_run(sctri,key,scale_sccaf,layer,added_metrics_kwargs=None):
    ...
    if issparse(adata.X) and not getattr(sctri,'sparse_compute',SPARSE_COMPUTE):
    ...
    cluster_to_metric = {}
    '''marker gene'''
    # read the flag off the object, not off module state: with the 'spawn' start
    # method a worker is a fresh interpreter and would not see a module-level
    # assignment made in the parent
    run_enrichment = getattr(sctri,'run_enrichment',None)
    ...
    '''SCCAF score'''
    cluster_to_metric['cluster_to_SCCAF'], confusion_sccaf = SCCAF_score(
        adata_to_compute,key,species,criterion,scale_sccaf,
        mode=getattr(sctri,'sccaf_mode',None))
...
def run_shapley(obs,query,reference,size_dict,data,mode,bonus):
    '''
    Score every annotation for every cell and pick the winner.

    Delegates to ``shapley.run_shapley_batch``. Upstream looped cell by cell and
    player by player, enumerating all 2**(n-1) coalitions each time; the batch
    engine evaluates the same sum in closed form over deduplicated score
    matrices. See the module docstring of ``shapley.py`` for the derivation.
    '''
    logger_sctriangulate.info('process {} need to process {} cells for shapley computing'.format(os.getpid(),data.shape[1]))
    return run_shapley_batch(obs,query,reference,size_dict,data,mode,bonus)


def run_shapley_legacy(obs,query,reference,size_dict,data,mode,bonus):
    '''Upstream per-cell implementation, kept for the equivalence tests.'''
    final = []
    intermediate = []
...
def run_assign(obs):
    '''
    Build the raw scTriangulate label ``<winning annotation>@<its cluster>`` per cell.

    Upstream did ``obs.iloc[i,:].loc[name]`` inside a per-cell python loop, which
    materialises a whole pandas Series per cell. Here each annotation column is
    gathered once and selected with a boolean mask, so cost is O(n_annotations)
    vectorised passes instead of O(n_cells) Series constructions. Same strings.
    '''
    logger_sctriangulate.info('process {} need to process {} cells for raw sctriangulte result'.format(os.getpid(), obs.shape[0]))
    final = obs['final_annotation'].to_numpy(dtype=object)
    assign = np.empty(obs.shape[0], dtype=object)
    for key in pd.unique(final):
        mask = final == key
        assign[mask] = key + '@' + obs.loc[mask, key].astype('str').to_numpy(dtype=object)
    obs['raw'] = assign
    return obs
