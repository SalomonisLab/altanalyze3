...
PALETTE_SEED = 0   # upstream drew the >102-cluster palette unseeded
...
def pick_n_colors(n,gradient=False,cmap=None):
    ...
    if gradient:
        ...
        if n <= 10:
            ...
            #
            # Upstream called np.random.choice(r433, size=n, replace=False), which
            # had two faults in exactly the range this branch exists for:
            #   * unseeded, so every run coloured the same clusters differently;
            #   * ValueError above 433 clusters, because the pool holds 433 entries
            #     and replace=False cannot draw more than that.
            # Both are fixed here. The palette is drawn with a fixed seed, and past
            # the pool size it repeats deterministically rather than raising. A
            # repeated colour above 433 clusters is unavoidable with a 433-colour
            # pool, and is better than a crash; the log says when it happens.
            pool = list(dict.fromkeys(r433))          # drop duplicate hex codes
            rng = np.random.default_rng(PALETTE_SEED)
            order = rng.permutation(len(pool))
            if n <= len(pool):
                _colors = [pool[i] for i in order[:n]]
            else:
                logger_sctriangulate.warning(
                    'pick_n_colors: {} clusters exceed the {} distinct colours available, '
                    'so colours repeat every {} clusters'.format(n, len(pool), len(pool)))
                _colors = [pool[order[i % len(pool)]] for i in range(n)]
    return _colors
