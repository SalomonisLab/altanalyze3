...
def reassign_pruning(sctri,abs_thresh=10,remove1=True,reference=None): #if you want to build viewer,set it to True
    ...
    for key in query:
        ...
        for i in range(marker.shape[0]):
            ...
            pool.extend(pick)
    # sorted, not list(set(...)): python randomises string hashing per process, so
    # upstream's gene column order (and therefore the PCA below) changed between runs
    pool = sorted(set(pool))
    adata_now = adata[:,pool].copy()

    ## standardise and reduce without materialising the centred matrix; see
    ## metrics.scaled_pca. Upstream densified here and used the randomized SVD.
    n_components = 30
    scoring, _pca_mode = scaled_pca(adata_now.X, n_components,
                                    random_state=PCA_RANDOM_STATE, mode=REASSIGN_PCA)
    ...
    if remove1:
        # Any 'pruned' label that is unique within a reference cluster gets folded
        # into that reference cluster's most abundant label. Upstream walked the
        # rows of each group with .iloc and wrote back through a chained
        # indexer, which is O(n_cells) python and raises SettingWithCopy; the
        # mask below picks exactly the same rows and writes the same values.
        bucket = []
        for ref,subset in modified_obs.groupby(by=reference,observed=True):
            subset = subset.copy()
            ...
            if len(exclude_clusters) > 0:
                mask = subset['pruned'].isin(exclude_clusters).values
                subset.loc[mask,'pruned'] = most_abundant_cluster
            bucket.append(subset)
...
def run_reference_pruning(chunk,reference,size_dict,obs):
    ...
    most_abundant_cluster = vc2.loc[vc2==vc2.max()].index[0]  # if multiple, just pick the first one
    exclude_clusters = vc2.loc[vc2==1].index
    if len(exclude_clusters) > 0:   # vectorised; upstream looped rows with chained .iloc assignment
        mask = subset['pruned'].isin(exclude_clusters).values
        subset.loc[mask,'pruned'] = most_abundant_cluster
    return subset
