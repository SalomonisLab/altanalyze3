...
def build_gene_model(df):
    ...
    if strand == '+':
        ...
        while p <= gene_end:
            try:
                ...
                unique_exon_ids = list(dict.fromkeys(position_ense[p-1]))
                associated_ense = '|'.join(unique_exon_ids)  
                ...
                if position_footprint[p] != running_profile:   
                    ...
                    subexon_identifier = 'E'+str(block_index)+'.'+str(segment_index)
                    unique_exon_ids = list(dict.fromkeys(position_ense[p - 1]))
                    associated_ense = '|'.join(unique_exon_ids) # because the current p is the first base in next segment, we are writing the previous segment and its associated ense
    ...
    else:   # negative strand, so backtrack
        string_stream = ''
        ...
        while p >= gene_start:
            try:
                ...
                if position_footprint[p] != running_profile:   
                    ...
                    unique_exon_ids = list(dict.fromkeys(position_ense[p+1]))
                    associated_ense = '|'.join(unique_exon_ids)  
                    anchor_position = p + 1  # fix: move this BEFORE writing
                    string_stream += '{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n'.format(ensg, subexon_identifier, chromosome, strand, anchor_position, previous_anchor, '', associated_ense)

                    running_profile = position_footprint[p]
                    segment_index += 1
                    previous_anchor = p  # new line to update the reference
                    p -= 1
                else:
                    p -= 1
        # write the last exon segment
